...
# === Sensor positions on pipe cross section (XY plane at Z=0) ===
...
# === Sensor orientation vectors (axes) ===
# Z axis: radially outward in XY plane
...
# X axis: right-handed system (cross product Y x Z)
sensor_x_dirs = np.cross(sensor_y_dirs, sensor_z_dirs)

arrow_length = 1.5  # For axis arrows
...
def load_csv_data(filename):
    with open(filename, newline='') as f:
        ...
        for row in reader:
            ...
            data.append(np.stack([bx, by, bz], axis=1))  # shape (8, 3)
    return np.array(timestamps), np.array(data)  # data shape: (n_samples, 8, 3)
